...
#T: Analiz tarihinden ne kadar süre önce ilk satın alma yapılmış. Haftalık. Son satın alma-İlk satın alma müşterinin yaşıdır.
...
'''
Buy till you die. 
‘Expected Number of Transaction’ için iki süreci olasılıksal olarak modeller.
Transaction Process (Buy) + Dropout Process (Till you die)

Transaction Process: Alive olduğu sürece, belirli bir zaman periyodunda, bir müşteri tarafından gerçekleştirilecek işlem sayısı transaction rate parametresi ile poisson dağılır. 
Bir müşteri alive olduğu sürece kendi transaction rate'i etrafında rastgele satın almaya devam edecektir. 
Transaction rate'ler her bir müşteriye göre değişir ve tüm kitle için gamma dağılır. (r,a) 

Dropout Process: Her bir müşterinin p olasılığı ile dropout rate (dropout probability)'i vardır.
...
from lifetimes import BetaGeoFitter
from lifetimes import GammaGammaFitter
#Modeli BetaGeoFitter ile kurduk ve alpha, a ve r değerlerine yani parametrelerine eriştik.
...
#Satın alma frekansını biliyoruz şimdi ise tahmini bu satışlardan ne kadar kazanacağız bilgisine erişeceğiz.
#Frequency ve monetary bilgisini verdik ve modeli fit ile tuttu.
ggf = GammaGammaFitter(penalizer_coef=0.01)
ggf.fit(cltv_df['frequency'], cltv_df['monetary_cltv_avg'])

#conditional_expected_average_profit ile de tahmini ne kadar getiri olacak hesapladık.


#**customer_lifetime_value** fonksiyonu ile modeli parametreleri girerek ulaşabiliriz:
...
#Artık elimde hem tahmini satış sayısı hem de tahmini satış getirisi var. Bunları kullanarak CLTV hesaplamaya geçebiliriz.
#ggf yani gamma gamma modeline bgf’i yani BG-NBD modelini bağlıyoruz. time-6 aylık periyotta tahmin yap dedik.
#Frekans bilgisini haftalık yani ‘W’ olarak verdik.
#Aylara göre de 0.01 indirim girdik.
...
cltv_df.head()
cltv_df.groupby('cltv_segment')['cltv'].median()
